...
void MiladyHash_2_create(uint soffset,uint coffset,Circom_CalcWit* ctx,std::string componentName,uint componentFather);
void MiladyHash_2_run(uint ctx_index,Circom_CalcWit* ctx);
...
uint get_total_signal_no() {return 890;}

uint get_number_of_components() {return 3;}

uint get_size_of_input_hashmap() {return 256;}

uint get_size_of_witness() {return 662;}
...
ctx->componentMemory[coffset].templateName = "MiladyHash";
ctx->componentMemory[coffset].signalStart = soffset;
...
MiladyHash_2_create(1,0,ctx,"main",0);
MiladyHash_2_run(0,ctx);
}
